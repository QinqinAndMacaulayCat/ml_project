...
The MLP is implemented as a small fully connected feed-forward network with ReLU hidden layers and a single linear output, trained with mean squared error loss using the Adam optimizer. In each rolling window, we re-fit the MLP on that window and evaluate it on the next year, using an internal validation split within the training window for early stopping. This setup keeps the MLP directly comparable to the other regression models while controlling training time, which is why we do not include it in the stacked regressor.
...
Across the regression models, tree-based ensembles deliver the strongest performance. The boosting regressor and stacking regressor, both based on LightGBM, achieve the lowest rolling-window MSE (around $8.6$--$8.9\times 10^{-5}$), with linear regression close behind at $9.4\times 10^{-5}$ and Elastic Net at $9.7\times 10^{-5}$. The tuned multilayer perceptron (MLP) has a somewhat higher MSE of $1.05\times 10^{-4}$ and MAE of $0.00352$, but attains the highest $R^2$ (0.064), indicating that it captures slightly more explainable variation in YTM changes at the cost of larger squared errors. Overall, the gap in MSE between the tree-based models and the linear/MLP benchmarks is modest but consistent across rolling windows, reinforcing that nonlinear structure matters but that tree ensembles are more effective than this relatively small MLP architecture for our feature set.
...
\section{Conclusion}
In this project, we explored a range of linear, tree-based, and neural-network models to forecast corporate bond YTM changes and to classify the sign of those changes. Using a rolling-window evaluation that mimics a realistic investment workflow, we compared models on mean squared error (MSE) for regression and on accuracy, precision, and recall for classification.

Using MSE as the main benchmark for the regression task, the LightGBM-based boosting and stacking regressors consistently deliver the lowest errors, with linear and Elastic Net regressions slightly behind and the MLP performing worst in terms of MSE. The MLP nevertheless attains the highest $R^2$, indicating that it captures somewhat more explainable variation in YTM changes at the expense of higher squared loss. Overall, the improvements in MSE from tree-based models over the linear and MLP baselines are modest but systematic across rolling windows, suggesting that nonlinear interactions matter but can be captured effectively by gradient-boosted trees without resorting to more complex neural architectures.

For the classification task, the boosting and stacking classifiers also outperform logistic and Elastic Net classification in accuracy, precision, and recall, again highlighting the value of tree-based ensembles when combining firm fundamentals, market variables, and macro indicators. Taken together, the results suggest that for this feature set and sample, LightGBM-style tree ensembles offer the best trade-off between predictive accuracy and model complexity for both regression and classification of corporate bond YTM changes.
...
\begin{table}[H]
\centering
\caption{Description of Macroeconomic Features}
\label{tab:macroeconomic_features}
\begin{tabular}{l p{10cm}}
\toprule
\textbf{Field Name} & \textbf{Description} \\
\midrule
\texttt{sp500} &
Monthly level of the S\&P 500 index, capturing overall equity market conditions and risk appetite. \\[0.5em]

\texttt{ir3m} &
Monthly average 3-month Treasury yield, representing short-term risk-free rates and monetary policy stance. \\[0.5em]

\texttt{ir10y} &
Monthly average 10-year Treasury yield, reflecting long-term discount rates relevant for bond valuation. \\[0.5em]

\texttt{vix} &
Monthly average VIX index, measuring market-implied volatility and broad risk aversion. \\[0.5em]

\texttt{gdp} &
Real GDP level or growth, aligned to each month, summarizing aggregate economic activity. \\[0.5em]

\texttt{cpi} &
Consumer Price Index (level or change), measuring inflation conditions relevant for nominal yields and real required returns. \\
\bottomrule
\end{tabular}
\end{table}
