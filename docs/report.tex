...
\begin{itemize}
    \item Time to Maturity (\texttt{tmt}): the remaining time to maturity of the bond in years.
    \item Coupon Rate (\texttt{coupon}): the annual coupon rate of the bond.
    \item \texttt{t\_spread}: the weighted average bid-ask spread of the bond which measures liquidity.
    \item \texttt{return\_eom}: bond return of last month.
    \item Credit Ratings: One-hot encoded variables for ratings from AAA to D.
    \item Rating Change Indicators: Binary variables (\texttt{upgrade}, \texttt{downgrade}).
\end{itemize}
...
\begin{itemize}
    \item \textbf{Stock Market Features:} \texttt{ret} (Monthly stock return), \texttt{vol} (volatility), \texttt{dvol} (Dollar trading volume), \texttt{turnover}, \texttt{bidask}, \texttt{numtrades}, \texttt{mktcap}, \texttt{price\_mean}.
    \item \textbf{Accounting-Based Derived Indicators:} \texttt{log\_atq}, \texttt{lev\_total} (total liabilities / total assets), \texttt{equity\_ratio}, \texttt{roa}, \texttt{profit\_margin}, \texttt{int\_coverage}, \texttt{mkt\_cap} (Accounting-based), \texttt{market\_to\_book}.
    \item \textbf{Growth Indicators (QoQ):} \texttt{atq\_growth} (Asset growth), \texttt{revtq\_growth} (Revenue growth), \texttt{niq\_growth} (Net income growth).
\end{itemize}
...
Features include: Equity Market Return (\texttt{SP500}), Short-Term Interest Rate (\texttt{IR3M}), Long-Term Interest Rate (\texttt{IR10Y}), Implied Volatility (\texttt{VIX}), Real Economic Activity (\texttt{GDP}), and Inflation (\texttt{CPI}).
...
We link a sector ETF based on GICS sector and use the ETF's monthly price and return.
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item \textbf{Stock + Fundamentals (PERMNO, date):} CRSP monthly stock aggregates are merged with Compustat monthly-filled accounting fundamentals using the common firm identifier (PERMNO) and month-end date.
        \item \textbf{Add Macroeconomic Variables (date):} Monthly macro data are merged using the calendar month-end date.
        \item \textbf{Add Sector ETF Information (industry code):} Each firm is mapped to its sector ETF using global industry classification, and monthly ETF returns/prices are merged accordingly.
        \item \textbf{Merge with Bond Data (CUSIP root, date):} Bonds are linked to issuers via the first six digits of CUSIP (issuer6). Only issuer–month pairs that appear in both datasets are retained to ensure valid alignment.
    \end{itemize}
    ...
    \item \textbf{Normalization}
    \begin{itemize}
        \item \textbf{(1) Rank-Normalized Features:} Includes stock features, fundamentals, derived ratios, growth variables, and bond-level numeric predictors. Each month, variables are transformed using:
        \begin{equation}
        \text{scaled\_rank} = 2 \times \frac{\text{rank}}{N} - 1
        \end{equation}
        producing values in $[-1, 1]$. For binary variables (e.g., upgrade/downgrade), ranks are assigned such that 1 maps to 1 and 0 maps to $-1$.
        
        \item \textbf{(2) Not Normalized (kept in raw form):} Includes macroeconomic indicators and yield-curve variables (e.g., \texttt{sp500\_ret}, \texttt{gdp\_gr}, \texttt{cpi\_infl}, \texttt{ir3m\_chg}, \texttt{ir10y\_chg}, \texttt{vix\_chg}, \texttt{s3m}, \texttt{term\_spread}). These variables are identical across all firms in a given month; rank-normalizing them collapses them into constants, eliminating their informational content. Therefore, they are kept in raw form to preserve meaningful macro signals.
    \end{itemize}
\end{enumerate}
...
When tuning is not involved or cross-validation is adopted, the validation set was combined with the training set to train the final model. For example, to predict YTM changes in 2012, we used data from 2002 to 2011 as the training set, data from 2012 as the validation set, and data from 2013 as the test set. Then, to predict YTM changes in 2013, we used data from 2003 to 2012 as the training set, data from 2013 as the validation set, and data from 2014 as the test set. This process was repeated until we predicted YTM changes in 2024.
...
Here, for the ElasticNet model, we tuned the hyperparameters alpha and l1\_ratio using 10-fold cross-validation on the training set.
...
Here, we also tuned hyperparameters for ElasticNet Classifier and Boosting Classifier using similar approaches as in regression task. For the Stacked Classifier, we combined predictions from Logistic Regression, ElasticNet Classifier, and LightGBM Classifier using logistic regression as the meta-model.
