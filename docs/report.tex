...
In this project, we explored various machine learning models to predict corporate bond yield to maturity changes using firm-level fundamentals, market-based risk factors, and macroeconomic indicators. In this project, we explored various machine learning models to predict corporate bond yield to maturity changes using firm-level fundamentals, market-based risk factors, and macroeconomic indicators. Our findings indicate that ensemble methods like LightGBM provide clear gains in the classification task, where they outperform traditional linear models in terms of accuracy, precision, and recall. For the regression task, however, even flexible nonlinear models such as LightGBM, model stacking, and the tuned MLP deliver only modest improvements in MSE and at best a relatively small positive out-of-sample $R^2$ (for the tuned MLP), while the other nonlinear models obtain $R^2$ values that are close to zero or slightly negative compared with a simple linear benchmark.
...
Multilayer Perceptron & 38 min 48 seconds & 3 min 16 sec \\
\bottomrule
\end{tabular}
\end{table}
\end{singlespace}
...
\subsubsection{B.2 Classification Confusion Matrix}
